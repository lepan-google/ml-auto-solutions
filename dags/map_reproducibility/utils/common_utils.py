...
def run_nemo_workload(
  ...
  Args:
    hypercomputer: The type of the GPU chips.
    model_id: The model ID.
    framework: The workload framework.
    precision: The precision used by the workload.
    metrics_model_id: The model ID used in the metrics table.
    num_gpus: Number of GPU used in the workload.
    num_steps: Number of the steps running by the workload.
    kueue_name: The name of the used kueue.
    config_model_name: The model name in the config file name.
    git_name: str =The github account username, this is used to download
    the repo change reference.
    git_email: The github email, this is used to download the repo change
    reference.
    storage_product: The storage product used in the workload.
    gcs_results_generator: True if enabling GCS run results generator.
    recipe_branch: The branch name of the recipe repo, use main branch by
    default.
    recipes_repo_change_refs: The change refs of the recipe repo.
    bq_writer_repo_change_refs: The change refs of the BQ writer repo.
    gcs_automation_repo_change_refs: The change refs of the gcs automation repo.
  ...
  with tempfile.TemporaryDirectory() as tmpdir:
    ...
    workload_image = workload_image if workload_image else get_image_version(
        framework)
